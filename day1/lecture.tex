...
\item \textbf{git pull} to take any changes from the repository. \textbf{git fetch} or \textbf{git remote update} may be better.
\end{itemize}
\end{itemize}
...
\frametitle{Quick Exercise}
    \begin{block}{EXERCISE}
        \begin{enumerate}
        \item Fork the \textbf{\href{https://github.com/topherbuckley/myFirstRepo}} repository using a browser.
        \item Clone the forked repository* to your local disk:
        \begin{lstlisting}
git clone git@github.com:<git_user_name>/myFirstRepo.git
        \end{lstlisting}
        or
        \begin{lstlisting}
git clone https://github.com/<git_user_name>/myFirstRepo.git
        \end{lstlisting}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item A fork is a copy of some other repo.
		\item We want to keep our fork up to date with the repo so we can merge changes we make to the original repo eventually. 
		\item Keep track of both the original and the fork via \textbf{git remote}
		\item Using \textbf{git log --all} or the longer syntax from before, we can see the status of each repo.
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{block}{EXERCISE}
		\begin{enumerate}
			\item \textbf{git remote add upstream git@github.com:topherbuckley/myFirstRepo.git} or
			\item \textbf{git remote add upstream https://github.com/topherbuckley/myFirstRepo.git}
			\item \textbf{git remote update} to download any changes from the remotes. Your fork is likely called "origin" and the original repo called "upstream"
			\item check this with \textbf{git remote -v} to show all remotes and \textbf{git log --all} to see how they all overlap.
			\item Let me make some updates to upstream and then you try \textbf{git fetch upstream} or \textbf{git remote update} then \textbf{git log --all} to see the changes
		\end{enumerate}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item A pull request is making a request to some github repo to merge in your changes.
